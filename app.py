...
def get_needed_data(
    ...
    if vessel_number not in vessel_log_cache:
        ...
        vessel_log_cache[vessel_number] = [
            get_vessel_logs(vessel_imo, start_time, end_time)
        ]
        ...
        if left_chunk_idx != right_chunk_idx:
            ...
            if left_chunk_idx is None:
                ...
                end_time = vessel_log_cache[vessel_number][right_chunk_idx][
                    -1
                ].ts.strftime('%Y-%m-%d %H:%M:%S')
                vessel_log_cache[vessel_number].insert(
                    0, get_vessel_logs(vessel_imo, start_time, end_time)
                )
                return 0
            elif right_chunk_idx is None:
                del vessel_log_cache[vessel_number][left_chunk_idx:length]
                start_time = vessel_log_cache[vessel_number][left_chunk_idx][
                    0
                ].ts.strftime('%Y-%m-%d %H:%M:%S')
                vessel_log_cache[vessel_number].append(
                    get_vessel_logs(vessel_imo, start_time, end_time)
                )
                return len(vessel_log_cache[vessel_number]) - 1
            else:
                del vessel_log_cache[vessel_number][
                ...
                vessel_log_cache[vessel_number].insert(
                    left_chunk_idx, get_vessel_logs(vessel_imo, start_time, end_time)
                )
                return left_chunk_idx


def extract_data_from_chunk(
    chunk: list[VesselLog], start_time_dt: datetime, end_time_dt: datetime
) -> list[VesselLog]:
    left_index = None
    right_index = None
...
def get_data(
    csv_algorithms: str, start_time: str, end_time: str, vessel_number: int = 125
):
    ...
    idx = get_needed_data(
        vessel_number, vessel_one.imo, start_time, end_time, start_time_dt, end_time_dt
    )
    needed_data = extract_data_from_chunk(
        vessel_log_cache[vessel_number][idx], start_time_dt, end_time_dt
    )
            ...
            case 'DR':
                ...
                for route in routes:
                    ...
                    for point in route['route']:
                        ...
                        simplified_route = dead_reckoning(
                            simplified_route, tolerance=2000
                        )
...
def index():
    return render_template(
        'index.html.jinja', algorithms=['Ours', 'SQUISH', 'SQUISH-E', 'DP', 'DR']
    )
