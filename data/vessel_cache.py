...
def get_chunk(vessel: Vessel, start_time: datetime, end_time: datetime) -> CacheChunk:
    ...
    for i, chunk in enumerate(chunks): # type: (int, CacheChunk)
        ...
        if chunk.from_date() <= end_time <= chunk.to_date():
            right_chunk_idx = i
        if left_chunk_idx is not None and right_chunk_idx is not None:  # No need to continue searching
            break

    if left_chunk_idx is None and right_chunk_idx is None:  # Both dates are outside cached chunks
        ...
        del chunks[0: right_chunk_idx + 1] # Remove all chunks up to and including right_chunk_idx
        ...
        end_time = chunks[right_chunk_idx].to_date()
        del chunks[left_chunk_idx: right_chunk_idx + 1] # Remove all chunks between left_chunk_idx and right_chunk_idx
        chunks.insert(left_chunk_idx, get_data_from_db(vessel, start_time, end_time))
        return chunks[left_chunk_idx]



def extract_segment_from_chunk(chunk: CacheChunk, start_time: datetime, end_time: datetime) -> list[VesselLog]:
    left_index = None
    right_index = None
